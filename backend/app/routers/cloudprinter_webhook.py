...
def _send_tracking_email(to_email: str,
    ...
    subject = f"Your order from Peekori {order_ref} has been shipped!"
    html = f"""
    <html>
    <head>
      <meta charset="UTF-8">
...
async def cloudprinter_webhook(
    ...
    if once.modified_count == 1:
        ...
        to_email = (order.get("customer_email") or order.get("email")
                    or "").strip() if order else "support@peekori.com"
        # to_email = "support@peekori.com"
